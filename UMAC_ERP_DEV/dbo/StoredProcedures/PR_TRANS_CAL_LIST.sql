...
-- 수정자 : 2024.09.02 강세미- 조회조건 추가(차량번호)
			2024.11.25 임현태- 조회조건 변경(입출고예정일 => 마감일자)
			2024.11.26 강세미- 운송비,용차비 둘다 없을 경우 운송비마스터 데이터 조회, 거래처별 탭 추가, 조회조건 입출고일자 없을경우 입출고예정일자로 조회
			2024.11.27 강세미- 조회조건 추가(상품구분)
			2024.12.05 강세미- 조회조건(기사명) 추가, 유맥 탭 추가
			2024.12.16 강세미- 운송구간 데이터타입 수정 NVARCHAR(3) -> NVARCHAR(6)
-- 수정일 : - 
-- 설 명  : 배차정산조회
-- 실행문 : 
EXEC PR_TRANS_CAL_LIST '', '', '20240901','20241126', '', '','','','','120001,240125','','transCalGrid'
*/
CREATE PROCEDURE [dbo].[PR_TRANS_CAL_LIST]
( 
	...
	@P_DRIVER_NAME				VARCHAR(20) = '',			-- 기사명
	@P_ACTIVE_TAB				NVARCHAR(20)				-- 활성화된 탭
)
AS
BEGIN
		...
		BEGIN
			WITH LIST AS (
				SELECT A.DELIVERY_DT,
					   ...
					   CASE WHEN ISNULL(A.TRANS_COST, 0) = 0 AND ISNULL(A.RENT_COST, 0) = 0 THEN dbo.GET_DELIVERY_PRICE('RENT_COST', B.DELIVERY_PRICE_SEQ, A.CAR_GB ) ELSE ISNULL(A.RENT_COST, 0) END AS RENT_COST,
					   CASE WHEN ISNULL(A.TRANS_COST, 0) = 0 AND ISNULL(A.RENT_COST, 0) = 0 THEN dbo.GET_DELIVERY_PRICE('TRANS_COST', B.DELIVERY_PRICE_SEQ, A.CAR_GB ) - dbo.GET_DELIVERY_PRICE('RENT_COST', B.DELIVERY_PRICE_SEQ, A.CAR_GB )
						    ELSE ISNULL(A.TRANS_COST, 0) - ISNULL(A.RENT_COST, 0) END AS FEE,
			...
			WHERE 1=1
			  ...
			  AND 1 = (CASE WHEN @P_DRIVER_NAME = '' THEN 1 WHEN @P_DRIVER_NAME <> '' AND A.DRIVER_NAME LIKE '%' + @P_DRIVER_NAME + '%' THEN 1 ELSE 0 END)																		
			  ...
			  AND 1 = (CASE WHEN @P_ACTIVE_TAB = 'transCalGrid' THEN 1															--전체
							WHEN @P_ACTIVE_TAB = 'transCalGrid2' AND A.VEN_CODE = 'UM10002' THEN 1								--동서매입
							WHEN @P_ACTIVE_TAB = 'transCalGrid3' AND A.VEN_CODE = 'UM20123' AND A.SCAN_CODE = '240125' THEN 1	--동서매출(호진물산) 배아박만
							WHEN @P_ACTIVE_TAB = 'transCalGrid4' AND (A.VEN_CODE IN ('UM10052', 'UM20132')) THEN 1				--청림
							WHEN @P_ACTIVE_TAB = 'transCalGrid5' AND A.VEN_CODE = 'UM20001' THEN 1								--신이억유통
							WHEN @P_ACTIVE_TAB = 'transCalGrid6' AND A.VEN_CODE = 'UM20005' THEN 1								--데버그린
							WHEN @P_ACTIVE_TAB = 'transCalGrid7'																--이외 거래처
