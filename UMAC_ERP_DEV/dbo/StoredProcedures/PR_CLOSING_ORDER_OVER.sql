...
EXEC PR_CLOSING_ORDER_OVER '[{"ORD_NO":"2241011003","CLOSE_REMARKS":null, "OVER_TYPE":"PRE"}]','admin'
EXEC PR_CLOSING_ORDER_OVER '[{"ORD_NO":"2241011003","CLOSE_REMARKS":null, "OVER_TYPE":"NEXT"}]','admin'

*/
CREATE PROCEDURE [dbo].[PR_CLOSING_ORDER_OVER]
( 
	...
	BEGIN TRY 
		...
		FETCH NEXT FROM CURSOR_REMARKS INTO @P_ORD_NO, @P_CLOSE_REMARKS, @P_OVER_TYPE
			...
			BEGIN
				...
				BEGIN
					-- 출고일 기준 전월 말일 세팅
					SELECT @PRE_MONTH_DT = CONVERT(VARCHAR(8),EOMONTH(DATEADD(MONTH, -1, DELIVERY_DEC_DT)), 112) 
					  FROM PO_ORDER_HDR 
				...
				BEGIN
					-- 출고일 기준 익월 1일 세팅
					SELECT @NEXT_MONTH_DT = CONVERT(VARCHAR(8),DATEADD(MONTH, 1, DATEFROMPARTS(YEAR(DELIVERY_DEC_DT), MONTH(DELIVERY_DEC_DT), 1)), 112) 
					  FROM PO_ORDER_HDR 
					 WHERE ORD_NO = @P_ORD_NO
