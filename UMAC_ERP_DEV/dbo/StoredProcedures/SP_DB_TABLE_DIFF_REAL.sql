...
-- 수정자 : 
-- 수정일 : 
-- 설 명  : UMAC_ERP_DEV TO 운영서버 UMAC_ERP 테이블 및 컬럼 비교
-- 실행문 : EXEC SP_DB_TABLE_DIFF_REAL
*/
CREATE PROCEDURE [dbo].[SP_DB_TABLE_DIFF_REAL]
AS
BEGIN
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;
	...
	--####################################################
							...
							SELECT C.* FROM UMAC_ERP_DEV.sys.columns AS C
									INNER JOIN UMAC_ERP_DEV.sys.tables T ON C.object_id = T.object_id
										WHERE T.name=@TABLE_NAME AND C.name NOT IN (
																					SELECT JC.name FROM [REAL_UMAC_ERP].UMAC_ERP.sys.columns JC
																						INNER JOIN [REAL_UMAC_ERP].UMAC_ERP.sys.tables JT ON JC.object_id = JT.object_id
				...
				BEGIN
						...
						INNER JOIN UMAC_ERP_DEV.sys.tables T 
							ON C.object_id = T.object_id
								WHERE T.name=@TABLE_NAME 
									AND C.name NOT IN (
														SELECT JC.name FROM [REAL_UMAC_ERP].UMAC_ERP.sys.columns JC 
															INNER JOIN [REAL_UMAC_ERP].UMAC_ERP.sys.tables JT 
					...
					BEGIN
							...
							PRINT 'DB : [REAL_UMAC_ERP].UMAC_ERP | TABLE NAME : ' + @TABLE_NAME + ' | 비매칭 컬럼 : ' + @COLNAME
	...
	PRINT ''
	PRINT ''
	PRINT '===== 운영서버 UMAC_ERP > UMAC_ERP_DEV ====='
	PRINT ''
	...
	--####################################################
				...
				IF EXISTS(
							SELECT C.* FROM [REAL_UMAC_ERP].UMAC_ERP.sys.columns AS C
									INNER JOIN [REAL_UMAC_ERP].UMAC_ERP.sys.tables T ON C.object_id = T.object_id
										WHERE T.name=@TABLE_NAME AND C.name NOT IN (
				...
				BEGIN
					...
					SELECT C.name FROM [REAL_UMAC_ERP].UMAC_ERP.sys.columns AS C 
						INNER JOIN [REAL_UMAC_ERP].UMAC_ERP.sys.tables T 
							ON C.object_id = T.object_id
								WHERE T.name=@TABLE_NAME 
									AND C.name NOT IN (
														SELECT JC.name FROM UMAC_ERP_DEV.sys.columns JC 
					...
					BEGIN
							SELECT  @COLNAME = COLUMN_NAME, @DATATYPE = DATA_TYPE, @CHARLEN = CHARACTER_MAXIMUM_LENGTH, @NUMERICLEN = NUMERIC_PRECISION,@NUMERIC_SCALE = NUMERIC_SCALE
							FROM [REAL_UMAC_ERP].UMAC_ERP.INFORMATION_SCHEMA.COLUMNS 
								WHERE TABLE_NAME = @TABLE_NAME AND COLUMN_NAME=@othercol
							ORDER BY ORDINAL_POSITION	
