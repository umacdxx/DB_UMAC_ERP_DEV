...
/*
-- 생성자 :	윤현빈
-- 등록일 :	2024.03.08PR_WMS_REPORT_HDR_LIST
-- 설 명  : 입출고 현황 리스트
-- 수정자 :	이동호
-- 수정일 :	2024.03.26
-- 설 명  : 튜닝(성능이슈 개선) 
-- 실행문 : EXEC PR_WMS_REPORT_HDR_LIST '','20240801','20240829','','','','','','','',100,0
*/
CREATE PROCEDURE [dbo].[PR_WMS_REPORT_HDR_LIST]
( 
...
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;
	...
	BEGIN TRY 
		...
		SELECT 
			ROW_NUMBER() OVER(ORDER BY T1.IO_GB DESC, T1.ORD_NO_PARENT DESC) + @P_PAGE_INDEX AS ROW_NUM,
			@TOT_CNT AS TOT_CNT,
			T1.*
		FROM (
			...
			SELECT 			
				...
				, CONCAT(A.ADDR, ' ', A.ADDR_DTL) AS ADDR
				, A.[STATUS]
				, A.ITM_CNT
			...
			FROM (
				...
				WHERE 1 = (CASE WHEN @P_IO_GB > 0AND A.IO_GB = @P_IO_GB THEN 1 WHEN @P_IO_GB = 0 THEN 1 ELSE 2 END)	
		...
		--			OR A.DOCUMENT_REQ_3 = I.CD_ID3 
		--			OR A.DOCUMENT_REQ_4 = I.CD_ID4)
		--		--LEFT OUTER JOIN CD_PARTNER_MST AS D ON A.VEN_CODE = D.VEN_CODE							
